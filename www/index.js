...
const callRadio = (command) => {
  ...
  fetch("/api/v1/command", {
    method: "POST",
    body: JSON.stringify({command, nonce}),
  })
  .then((response) => {
  ...
  setInterval(() => {
    ...
    dispatch((state) => {
      if (state.powerState !== state.reportedPower) {
        callRadio(state.powerState === 'on' ? 'power_on' : 'power_off');
        return {
          reportedPower: state.powerState,
        }
      } else if (state.muteState !== state.reportedMute) {
        callRadio(state.muteState === 'on' ? 'mute_on' : 'mute_off');
        return {
        ...
        callRadio(diff == 1 ? 'volume_up' : 'volume_down');
        return {
    ...
    dispatch((state) => {
      if (state.powerState === 'off') {
        ...
        return {
          selection: source,
          powerState: 'on',
          reportedPower: 'on', // disable remote event.
          muteState: 'off',
          reportedMute: 'off',// disable remote event.
        ...
        return {
          selection: source,
          reportedSource: null, // Force update.
          visible: source + 'Inside'
        }
      }
    });
